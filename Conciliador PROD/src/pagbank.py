...
def padronizar_erp(df):  # Função para padronizar o DataFrame do ERP
    ...
    df = df.rename(columns={
        ...
        "Ch Criação": "CHAVE CRIAÇÂO ERP",
        "Chave": "CHAVE ERP",
        "Pessoa do Título": "PESSOA DO TITULO",
    ...
    st.write("COLUNAS ERP:", df.columns.tolist())
    ...
    df["VALOR LIQUIDO ERP"] = converter_valor(df["VALOR LIQUIDO ERP"])  # Converte valor líquido
    df["TAXA  ERP"] = converter_valor(df["TAXA  ERP"])  # Converte taxa

    # extrai número da parcela e total de parcelas do campo Número ERP
    parcelas = df["NUMERO ERP"].str.extract(r"-(\d+)/(\d+)")  # Extrai parcelas com regex
    ...
    df["NUMERO ERP"] = df["NUMERO ERP"].fillna("").astype(str).str.replace(r"-(\d+)/(\d+)", "", regex=True)

    # garante que Número ERP seja string pois contém texto estruturado
    df["NUMERO ERP"] = df["NUMERO ERP"].astype(str)  # Converte para string
...
def conciliar_pagbank_erp(df_pagbank_padrao, df_erp_padrao, tolerancia_dias=3, tolerancia_valor=0.30):  # Função de conciliação
    ...
    def calcular_score(linha, row):  # Função para calcular score de similaridade
        ...
        val = abs(linha["VALOR LIQUIDO ERP"] - row["Valor Líquido Pagbank"])  # Diferença em valor
    ...
    for i, row in df_pagbank_padrao.iterrows():  # Itera sobre cada linha do PagBank
        ...
        match = df_erp_padrao[
            ...
            (df_erp_padrao["PESSOA DO TITULO"].str.contains("pagseguro", case=False, na=False)) &  # Contém PagSeguro
            ...
            (df_erp_padrao["VALOR LIQUIDO ERP"] == row["Valor Líquido Pagbank"]) &  # Valor igual
        ...
        if not match.empty:  # Se encontrou match
            ...
            df_pagbank_padrao.loc[i, colunas_retorno] = [
                ...
                linha["CHAVE ERP"],
                linha["VALOR LIQUIDO ERP"],
                linha["DATA DA TRANSAÇÃO ERP"],
                linha["PESSOA DO TITULO"]
            ]  # Preenche colunas
        ...
        candidatos = df_erp_padrao[
            (~df_erp_padrao["Usada"]) &  # Não usada
            (abs(df_erp_padrao["VALOR LIQUIDO ERP"] - row["Valor Líquido Pagbank"]) <= tolerancia_valor) &  # Valor dentro tolerância
